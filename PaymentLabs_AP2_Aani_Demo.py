...
- FIX: Added unique keys for all buttons (avoids StreamlitDuplicateElementId).
"""
...
# ---------------------------
    ...
    if st.button("Next Step → Mandate Registry", key="to_registry"):
        go_to("Registry")
        st.rerun()
...
# ---------------------------
    ...
    if st.button("Next Step → Risk & AML Check", key="to_riskaml"):
        go_to("RiskAML")
        st.rerun()
...
# ---------------------------
    ...
    if not mandates:
        ...
        if st.button("Run Risk Screening", key=f"riskcheck_{sel_id}"):
    ...
    if st.button("Next Step → Payment Execution", key="to_payment"):
        go_to("Payment")
        st.rerun()
...
# ---------------------------
    ...
    if not available:
        ...
        if st.button("Execute Payment", key=f"execute_{sel_id}"):
    ...
    if st.button("Next Step → Audit Trail", key="to_audit"):
        go_to("Audit")
        st.rerun()
